...
def main():
    images = sorted(glob.glob("images/*.jpg"), key=lambda p: int(os.path.splitext(os.path.basename(p))[0]))

    # Parameters
    fps = 8.0  # ~0.125s per frame (slow enough to appreciate each photo)
    face_height = 400
    video_out_size = (1280, 720)
    manual_mode = False  # Set True only for interactive use
    ...
    for i, file_name in enumerate(images):
        ...
        if img is None:
            print(f"[{i+1}/{total}] Could not read {file_name}, skipping.")
            skipped += 1
            continue
        ...
        if len(face) == 0:
            print(f"[{i+1}/{total}] No face found in {file_name}, skipping.")
            skipped += 1
            continue

        print(f"[{i+1}/{total}] Processing {file_name} ✓")
    ...
    try:
        ...
        pass  # headless environment, no GUI to destroy
    print(f"\nDone! {written} frames written, {skipped} skipped → video.avi")
